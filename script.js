document.addEventListener('DOMContentLoaded', () => {
  ...
  /* -------------------------------------------------------------
     7. DYNAMIC FOOD MENU
     Os dados vêm de menu-data.js (fonte única, carregado antes deste
     script). Fallback para lista vazia caso o arquivo não carregue.
  ------------------------------------------------------------- */
  const menuItems = (typeof MENU_ITEMS !== 'undefined') ? MENU_ITEMS : [];
  ...
  if (cardapioGrid) {
    ...
    const renderMenu = () => {
      ...
      // Controlar exibição do botão "Ver mais" (opcional — pode não existir)
      if (verMaisContainer) {
        verMaisContainer.style.display = filteredItems.length > visibleCount ? 'flex' : 'none';
      }
    };
